...
def usage():
    print "Usage: -----> python DivideIC_xml_txt.py pathXml pathTXT Output"
# python DivideIC_xml_txt.py InputFile/EcalIntercalibConstants_GR10_H_V6.xml 2010/IC.txt 2010/ABSIC_ResidsualFree.root
def readCellId(node):
    ieta =-1
    iphi =-1
    ix   =-1
    ...
    for xtal in doc.getElementsByTagName("cell"):
        ...
        for value in xtal.childNodes:  
            if (value.nodeType == value.ELEMENT_NODE and value.nodeName == "Value" ):
                ...
                #           else:
                #              NewIcFile.write( str(IC_EB_1[find_EB1][0]) + ' ' + str(IC_EB_1[find_EB1][1]) + ' 0 -1 999.\n'  )
                ...
                #           break
                #    if not(Founded): print "WARNING!! IC not founded, EB " + str(int(id[1])) + " " + str(id[2])
                if (id[0]=='EE'):
                    Founded=False
                    for find_EE1 in range(len(IC_EE_1)):
                        if(int(IC_EE_1[find_EE1][0])==int(id[1]) and int(IC_EE_1[find_EE1][1])==int(id[2]) and str(IC_EE_1[find_EE1][2])==str(id[ int(len(id))-1 ])):
                           ...
                           if(float(IC_EE_1[find_EE1][3])!=-1):
                              ...
                              NewIcFile.write( str(IC_EE_1[find_EE1][0]) + ' ' + str(IC_EE_1[find_EE1][1]) + ' ' + str(IC_EE_1[find_EE1][2]) + ' -1 999.\n' )
                           ...
                           break
                    if not(Founded): print "WARNING!! IC not founded, EE " + str(id[1]) + ' ' + str(id[2]) + ' ' + str(id[3])
