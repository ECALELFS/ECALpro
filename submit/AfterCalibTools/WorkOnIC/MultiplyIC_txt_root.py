...
def WriteTXT(options, hEB,hEEm,hEEp,name,errorType,whichIC,hEB_1=None,hEEm_1=None,hEEp_1=None):
...
def AverageGlobally(options, hEB,hEEm,hEEp,name,errorType,hEB_1=None,hEEm_1=None,hEEp_1=None):
    outputfile = open( name, 'wr+' )
    outputfile.write("#iEta(ix) iPhi(iy) 0(iZ) IC toterr staterr systerr chi2\n")
    if (not options.exclude_EB):
        #EB
...
def AverageEtaRing(options, hEB,hEEm,hEEp,name,EtaList,errorType,hEB_1=None,hEEm_1=None,hEEp_1=None):
    outputfile = open( name, 'wr+' )
    outputfile.write("#iEta(ix) iPhi(iy) 0(iZ) IC toterr staterr systerr chi2\n")
    if (not options.exclude_EB):
        #EB
...
AverageGlobally(options, NewcalibMap_EB,NewcalibMap_EEm,NewcalibMap_EEp,name,"ErrorFromMyIC",EBIC_syst,EEmIC_syst,EEpIC_syst)
#Average to 1 PerEtaRing
print 'Executing AverageEtaRing'
name = OutputF + "/IC_fromECALpro_Absolute_EtaRing1.txt"
AverageEtaRing(options, NewcalibMap_EB,NewcalibMap_EEm,NewcalibMap_EEp,name,EtaList,"ErrorFromMyIC",EBIC_syst,EEmIC_syst,EEpIC_syst)
#Test the average
print 'Now the final test...'
TEST_average(options)
print 'THE END!'
f.cd()
f.Write()
f.Close()
