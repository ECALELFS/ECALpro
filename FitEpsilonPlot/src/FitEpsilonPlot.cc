...
//-----------------------------------------------------------------------------------
  ...
  //sprintf(line,"N_{bkg}: %.0f #pm %.0f", Nbkg.getVal(), Nbkg.getError() );
  //lat.DrawLatex(xmin,yhi-3.*ypass, line);


/*
  if(outname != 0) {
   //sprintf(line,"%s/%s%s", outeps, outname, ".eps");
   //c0->SaveAs(line);
    sprintf(line,"%s/%s%s", outpng, outname, ".png");
    c0->SaveAs(line);
  }
  if(res) res->Print();
*/
