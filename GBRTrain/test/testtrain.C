...
void testtrain() {
  ...
  train->SetTrainingCut("ph.ispromptgen && ph.isbarrel && ph.hasphoton && ph.pt>25 && evt%2==0");
  train->SetMinEvents(200);
  train->SetShrinkage(0.1);
    
 
  train->SetTargetVar("ph.gene/ph.scrawe");
...
/*  varsf->push_back("ph.etaseed-ph.sceta");
  ...
  TFile *fout = new TFile("fgbrtraintest.root","RECREATE");    
